...
export const HomeView: React.FC<HomeViewProps> = ({ onNavigate }) => {
  ...
  const handleCreateProject = (type: 'tv-30' | 'tv-60' | 'film') => {
    createProject(type);
    onNavigate('workshop');
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
            <Button onClick={() => handleCreateProject('tv-30')}>
              <Plus className="h-4 w-4 mr-2" />
              30-Min Show
            </Button>
            <Button onClick={() => handleCreateProject('tv-60')}>
              <Plus className="h-4 w-4 mr-2" />
              60-Min Show
            </Button>
            <Button onClick={() => handleCreateProject('film')}>
              <Plus className="h-4 w-4 mr-2" />
              Feature Film
            </Button>
          </div>
